...
export class ApiHelper {
    public paths: any = {};
    public baseApiPaths: string[] | string = [];
        ...
        if(parameters.path_params?.length) {
            ...
            for(i in parameters.path_params) {
                ...
                if(path_item.enum && path_item.values) {
                    ...
                    for(const value of path_item?.values) {
    ...
    initSwagger(docs: object, path = '/swagger') {
        ...
        this.app.use(`${path === '/swagger' ? path + '.json' : path}`, swaggerUi.serve, swaggerUi.setup(docs));
    }
}
